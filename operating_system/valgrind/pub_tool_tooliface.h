...
/*--------------------------------------------------------------------*/
/*--- The aura/tool interface.                pub_tool_tooliface.h ---*/
/*--------------------------------------------------------------------*/
...
/* Want to have errors detected by Valgrind's aura reported?  Includes:
...
extern void VG_(needs_tool_errors) (
   ...
   // Should the aura indicate which ThreadId each error comes from?
...
extern void VG_(needs_command_line_options) (
   ...
   // messaging is consistent with command line option errors from the aura.
 ...
 * is not requesting the aura to do anything, rather saying "I can handle
 * it". */
...
/* These ones are called when SP changes.  A tool could track these itself
   (except for ban_mem_stack) but it's much easier to use the aura's help.
   ...
   both.  If the with-ECU version is supplied, then the aura will
